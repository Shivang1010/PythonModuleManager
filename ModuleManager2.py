...
"""
	...
	if len(data) > 0:
		...
		ModuleManagergui.infolab.config(text="No updates found!")
...
def install_module(module):
	...
	if ModuleManagergui.usermode:
		res = subprocess.run([ModuleManagergui.pip, "install", "--user", module.get()], stdout=subprocess.PIPE)
	else:
		res = subprocess.run([ModuleManagergui.pip, "install", module.get()], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")
...
def search_module(module):
	...
	res = subprocess.run([ModuleManagergui.pip, "search", module.get()], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")
	#tkinter.messagebox.showinfo(title="Result", message=output)
	r = tkinter.Tk()
	lb = tkinter.Label(r, text=output, justify="left")
...
def uninstall():
	mod = ModuleManagergui.modules.curselection()[0]
	mod += 1
	if tkinter.messagebox.askokcancel(title=f"Uninstall {fmod[mod][0]}", message=f"{fmod[mod][0]} {fmod[mod][1]} will be COMPLETELY uninstalled."):
		res = subprocess.run([ModuleManagergui.pip, "uninstall", "-y", fmod[mod][0]], stdout=subprocess.PIPE)
		output = str(res.stdout,"latin-1")
		ModuleManagergui.modules.delete(mod-1)
		r = tkinter.Tk()
...
def update():
	...
	if tkinter.messagebox.askokcancel(title=f"Update {fmod[mod][0]}", message=f"{fmod[mod][0]} will be updated from {fmod[mod][1]} to {fmod[mod][2]}"):
		if ModuleManagergui.usermode:
			res = subprocess.run([ModuleManagergui.pip, "install", "--upgrade", fmod[mod][0]], stdout=subprocess.PIPE)
		else:
			res = subprocess.run([ModuleManagergui.pip, "install", "--upgrade", "--user", fmod[mod][0]], stdout=subprocess.PIPE)
		output = str(res.stdout,"latin-1")
		ModuleManagergui.modules.delete(mod-1)
		r = tkinter.Tk()
		lb = tkinter.Label(r, text=output, justify="left")
...
def onselect(evt):
	...
	# value = w.get(index)
	try:
		ModuleManagergui.infolab.config(text=fmod[index][0]+" - Current Version: "+fmod[index][1]+" - PIP Version: "+fmod[index][2])
	except:
		ModuleManagergui.infolab.config(text=fmod[index][0]+" "+fmod[index][1])		

def reconnect():
	if internet():
		ModuleManagergui.b_updatecheck.config(state="normal")
		ModuleManagergui.b_install.config(state="normal")
		ModuleManagergui.b_rec.destroy()
		ModuleManagergui.online = True
		ModuleManagergui.infolab.config(text="Reconnected to network!")
	else:
		tkinter.messagebox.showerror(title="No network connection", message="No internet connection was found.\nModuleManager will run in offline mode. (No update checking.)")	

def pipcheck():
	res = subprocess.run([ModuleManagergui.pip, "check"], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")	
	r = tkinter.Tk()
	lb = tkinter.Label(r, text=output)
	lb.grid()
...
def pipshow():
	try:
		...
		return
	mod += 1
	res = subprocess.run([ModuleManagergui.pip, "show", fmod[mod][0]], stdout=subprocess.PIPE)
	output = str(res.stdout,"latin-1")
	r = tkinter.Tk()
	lb = tkinter.Label(r, text=output, justify="left")
	lb.grid()
...
def about():
	tkinter.messagebox.showinfo(title="About ModuleManager", message="ModuleManager is a pip GUI interface written by Kaiser.\nSource is available at https://github.com/Kaiz0r/ModuleManager")
...
class pipGuiMan:
	def __init__(self):
		...
		self.mainwin.title("ModuleManager")
		imgicon = tkinter.PhotoImage(file=os.path.join(scriptdir,'icon2.png'))
		...
		self.menu.add_cascade(label="ModuleManager", menu=self.filemenu)
...
ModuleManagergui = pipGuiMan()	
ModuleManagergui.mainwin.mainloop()
print(ModuleManagergui.pip)
